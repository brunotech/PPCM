...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward_embed(self, input_ids, position_ids=None, token_type_ids=None, past=None):
        ...
        if token_type_ids is not None:
            ...
            token_type_embeds = 0
        return inputs_embeds + position_embeds + token_type_embeds
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None, past=None, task_id=-1, kl_weight=0):
        ...
        if lm_labels is not None:
            ...
            if kl_weight>0:
                ...
                dialgpt_logits = self.lm_head(hidden_states)
                dialgpt_shift_logits = dialgpt_logits[:, :-1].contiguous()
                logsoftmax = nn.LogSoftmax(dim=1)
                kl_fct = nn.KLDivLoss(reduction='batchmean')
                source_dist = logsoftmax(shift_logits.view(-1, shift_logits.size(-1)))
                target_dist = logsoftmax(dialgpt_shift_logits.view(-1, dialgpt_shift_logits.size(-1)))
                # print(source_dist)
                # print("==============")
                # print(target_dist)
    ...
    def forward_embed(self, inputs_ids, position_ids=None, token_type_ids=None, past=None):
        return self.transformer.forward_embed(
            inputs_ids, position_ids, token_type_ids, past
        )
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, mc_token_ids, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None, past=None):
        ...
        return losses or (lm_logits, mc_logits, presents)
